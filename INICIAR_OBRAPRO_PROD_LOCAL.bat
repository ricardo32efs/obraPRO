...
chcp 65001 >nul
title Obra Pro - Servidor local (producción)
cd /d "%~dp0"

echo.
echo === Obra Pro (compilado, como en internet) ===
echo Carpeta: %CD%
echo.
...
echo Abrí en el navegador: http://127.0.0.1:4173  (o http://localhost:4173^)
echo IMPORTANTE: no cierres esta ventana negra mientras uses la web.
echo La primera vez puede tardar unos segundos mientras compila ^(npm run build^).
echo.
...
if not "!EXITCODE!"=="0" (
  ...
  echo Si el puerto 4173 está ocupado, cerrá otras ventanas de Obra Pro o reiniciá la PC.
)
pause
exit /b !EXITCODE!
